...
class Disbursement_performance extends CI_Controller{
	...
	public function index($month = 0, $year = 0,$pdf = 0){ 
		$this->load->model('disbursement_performance_model'); 
		...
		$all_details = $this->disbursement_performance_model->get_details($date_check,$date,$date_check1);
				
		if(! $pdf){
			$this->page->title      = "Mombo Loan System | Monthly Disbursement Performance";
		 
			$this->page->name       = 'mombo-monthly-disbursement-performance';
			...
			$this->sendResponse($this->msg,array(
		    	...
		    	$this->header => $this->msg,
					...
					'mombo/templates/disbursement_performance_view' => $this->msg,
