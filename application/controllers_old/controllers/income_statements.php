...
class Income_statements extends CI_Controller{
	...
	public function index($month = 0, $year = 0,$pdf = 0,$type = 'A'){ 
		...
		$other_income_accts = $this->statements_model->get_other_income_accts();
		$other_income_array = array(); 
		foreach($other_income_accts as $other_income_acct){ 
			$other_income_array[] = $this->statements_model->get_other_incomes($other_income_acct["id"],$date);
		}
		//var_dump($other_income_array); exit;
		...
		if(! $pdf){
			...
			$this->msg['other_income_array'] = $other_income_array;
