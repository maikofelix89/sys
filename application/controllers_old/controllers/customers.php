...
class Customers extends CI_Controller{ 
	  ...
	  public function getclient(){
	         ...
	         $criteria = $this->input->get_post('criteria');
			 $customers = $this->loan->get_clients_by_criteria($criteria);
			 $count = 0;
			 $result = '';
			 if(0 == $customers){
				$result = 0;
				...
				foreach($customers as $customer){
					if(0 == $count){
						$result = $customer['id'].'*'.$customer['name'].' '.$customer['other_names'].'*'.$customer['idnumber'];
					}
					else{
						$result .= '~'.$customer['id'].'*'.$customer['name'].' '.$customer['other_names'].'*'.$customer['idnumber'];
					}
	  ...
	  public function getclientloans(){
	         ...
	         $client_id = $this->input->get_post('customer_id');
			 ...
			 if(null != $customer && in_array($action,array('activate','accept','reject','deactivate'))){ //check if customer record was found and action supported
				   ...
				   switch(strtolower($action)){
							...
							case 'accept': //set variables for customer registration accept action
								 ...
								 $message     = "Do you really want to approve this customer account?";
							...
							case 'reject': //set variables for customer registration reject action
								 ...
								 $message     = "Do you really want to reject this customer account?";
	  ...
	  public function viewcustomerdetails(){
		    ...
		    $this->sendResponse($this->msg,array(
					...
					'mombo/templates/customer' => $this->msg,
	  ...
	  public function customerloanstatement($loan_id,$customer_id,$pdf = 0){
		...
		if($pdf){
			if($customer->num_rows() > 0 ){ 
			   ...
			   $content .= '<table>
			   	<tr class="muted">
					...
					//echo $content;
					$this->customerloanpdf($content,$loan_id.'_'.$file_name);
					
				 }else{
                	 $this->customerloanpdf('No customer found!');
				} 
			...
			$this->sendResponse($this->msg,array(
				 ...
				 'mombo/templates/customer_loan' => $this->msg,
	  ...
	  public function customerloanpdf($content,$filename = 'Loan statement'){
	  	//echo 'hehehehe'; exit;
	  ...
	  public function birthdays(){
				...
				$today = $this->customer_model->get_today_birthdays();
				$today1 = $this->customer_model->get_upcoming_birthdays(1); 
				$today2 = $this->customer_model->get_upcoming_birthdays(2); 
				$today3 = $this->customer_model->get_upcoming_birthdays(3); 
				$today4 = $this->customer_model->get_upcoming_birthdays(4); 
				$today5 = $this->customer_model->get_upcoming_birthdays(5); 
				$today6 = $this->customer_model->get_upcoming_birthdays(6); 
				...
				$this->page->title      = "Mombo Loan System | Customer Birthdays";
				 
				$this->page->name       = 'mombo-customer-birthdays';
			...
			public function addcharge(){
				...
				if($this->loan_model->get_charge($loan_id,$charge_name, date('Y-m'))){
					redirect(base_url().'customers/viewcustomerdetails/?customer_id='.$cust_id.'/?message=The charge had already been added previously&message_type=warning');
				}
				...
				if($this->input->get_post('submit',TRUE)){
					...
					if($amount <= 0){
						redirect(base_url().'customers/viewcustomerdetails/?customer_id='.$cust_id.'/?message=Amount should be greater than zero&message_type=warning');
					}
					...
					if(!$this->loan_model->insert_charge_fee($loan_transaction_code,$charge_name,$loan_id,$amount)){
						redirect(base_url().'customers/viewcustomerdetails/?customer_id='.$cust_id.'/?message=Error has occured. Please try again later.&message_type=warning');
					}
					else{
						$this->loan_transactions_model->add_transaction($loan_id,$charge_name,'-',0,$amount);
						redirect(base_url().'customers/viewcustomerdetails/?customer_id='.$cust_id.'/?message=The charge has been added successfuly&message_type=info');
					}
				}
