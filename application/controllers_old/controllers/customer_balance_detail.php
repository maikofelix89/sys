...
class Customer_balance_detail extends CI_Controller{
	...
	public function index($date = 0,$pdf = 0){ 
		$this->load->model('customer_balance_detail_model'); 
		...
		$balance_details = $this->customer_balance_detail_model->get_balance_details($date);
				
		if(! $pdf){
			$this->page->title      = "Mombo Loan System | Customer Balance Detail";
		 
			$this->page->name       = 'mombo-customer-balance-detail';
			 
			$this->msg['page']      = $this->page;
			 
			$this->msg['balance_details'] = $balance_details;
			...
			$this->sendResponse($this->msg,array(
		    	...
		    	$this->header => $this->msg,
					...
					'mombo/templates/customer_balance_detail_view' => $this->msg,
