...
class Accounts extends CI_Controller{
		...
		public function index(){
				...
				if($criteria && in_array($criteria,array('expense','income','other'))){
						...
						switch(strtolower($criteria)){
								...
								case 'income': $is_income = 1; $criteria = 'income'; break;
		...
		/******Account CRUD******/
				...
				public function accountsummary($to_acct = 0){
					...
					if($account_id || $sub_account_id){ 
							...
							if( ($to_acct == 1) || ($to_acct == 2) || ($to_acct == 17) ){ 
								...
								$other_account = $this->transaction_model->get_account_name($to_acct);
								...
								$this->transaction_model->insert_new_transaction($object,$payment_code,$to_acct,1,$description1,$date,$amount_to_transfer,$user_id,$payment_code);
								$this->transaction_model->update_balance($account_id,$amount_to_transfer,0);
								$this->transaction_model->update_balance($to_acct,$amount_to_transfer,1);
							...
							if(null != $account){
									...
									foreach($trans->result('transaction_model') as $transaction){
										...
										if($customer_id == 0){
											...
											$description = $transaction->description.", ".$customer_name;
										}
										$acct_bal[$transaction->payment_code] = $transaction->acct_bal;
										$amounts[$transaction->payment_code] = $transaction->amounttransacted;
										
										$transactions[] = $transaction->payment_code.'*'.$transaction->date.'*'.$description.'*'.$transaction->transactiontype.'*'.$transaction->amounttransacted.'*'.$transaction->acct_bal.'*'.$transaction->ref_num;
									...
									redirect(base_url().'accounts/?message=account doesn\'t exist&message_type=warning');
		...
		/******Account Activity******/
		        ...
		        public function transfercash($transfer_from,$parent_id=0,$approve=-2){
					$this->load->model('account_model');
				     if( ($this->input->post('transfer_from_bank')) || ($this->input->post('transfer_from_cash')) || $approve > -2 ){
					 
							$from_account_id = (int) $transfer_from;
							
							$to_account_string   = $this->input->get_post('payee');
							...
							if($from_account_id == 1 || $from_account_id == 2){
							...
							if($approve == -2){
								...
								$transfer['to_account_string'] = $to_account_string;
								$transfer['amount'] = $amount;
								$transfer['reference_number'] = $reference_number;
								$transfer['description'] = $description;
								$transfer['date'] = $date;
								$transfer['parent_id'] = $parent_id;
								...
								redirect(base_url().'accounts/?message=Transfer has been successful. It awaits approval&message_type=success');
								}else{
									redirect(base_url().'accounts/?message=Error occured! Please retry&message_type=error');
								}
							}
							...
							if($approve > -2){
								...
								$update_transfer['id'] = $id;
								$update_transfer['approved'] = $approve;
								$updated  = $this->transfers->update_status($id,$approve);
								
								if($updated){
									if($approve == -1){
										redirect(base_url().'finance_transfers/?message=Transfer rejected! Please retry&message_type=success');
									}
								}
								else{
									redirect(base_url().'finance_transfers/?message=Error occured! Please retry&message_type=error');
								}
							...
							if($parent_id == 0){
								...
								$from_account = $this->sub_account->findOne($from_account_id);
							}
							...
							if($check){
								if($parent_id == 0){
									...
									$this->account_model->update_account_balance($parent_id,$amount,0);
									$this->account_model->update_sub_account_balance($from_account_id,$amount,0);
								}
								$to_account_id = 0;
								...
								if($parent_id == 0){
									$this->account_model->insert_transaction($obj,$t_code,$from_account_id,0,$t_type_d,$description_d,$date,$amount,$user,$payment_code,$reference_number);//debit
								}
								else{
									$this->account_model->insert_transaction($obj,$t_code,$parent_id,$from_account_id,$t_type_d,$description_d,$date,$amount,$user,$payment_code,$reference_number);//debit
								}
								$this->account_model->insert_transaction($obj,$t_code,$acct_to,$sub_acct_to,$t_type_c,$description_c,$date,$amount,$user,$payment_code,$reference_number);//credit
								
								redirect(base_url().'accounts/?message=Transfer has been approved&message_type=success');
							}
							else{
								$id = $this->input->get('id');
								$updated  = $this->transfers->update_status($id,0);
