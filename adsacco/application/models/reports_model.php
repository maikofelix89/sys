...
class Reports_model extends CI_Model {
	...
	public function get_interests_a($loan_product_id,$date,$date_a){
		...
		if($loan_product_id == 2){
			$this->db->select("id");
			$this->db->where("loan_product = ".$loan_product_id." AND request_status=2 AND loan_issue_date LIKE '".$date."%'");
			$query = $this->db->get("loans");
			$loan_ids = $query->result_array();
			foreach($loan_ids as $loan_id){
	...
	public function get_future_interests_a($loan_product_id,$date_check1,$date_plus){
		$interest = 0;
	...
	public function get_new_future_interests_a($loan_product_id,$date_check1,$date_plus,$date){
		$interest = 0;
